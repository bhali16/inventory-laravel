        ...
        <div class="card-body">
            <form method="POST" action="{{ route('admin.products.update', [$product->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('product_code') ? 'is-invalid' : '' }}" type="number"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('product_name') ? 'is-invalid' : '' }}" type="text"
                        name="product_name" id="product_name" value="{{ old('product_name', $product->product_name) }}"
                        required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('product_mfg') ? 'is-invalid' : '' }}" type="text"
                        name="product_mfg" id="product_mfg" value="{{ old('product_mfg', $product->product_mfg) }}">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('product_price') ? 'is-invalid' : '' }}" type="number"
                        name="product_price" id="product_price" value="{{ old('product_price', $product->product_price) }}"
                        step="0.01" required>
                ...
                <div class="form-group">
                    ...
                    @foreach (App\Models\Product::PRODUCT_TYPE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('product_type') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="product_type_{{ $key }}"
                                name="product_type" value="{{ $key }}"
                                {{ old('product_type', $product->product_type) === (string) $key ? 'checked' : '' }}
                                required>
                            <label class="form-check-label"
                                for="product_type_{{ $key }}">{{ $label }}</label>
                        </div>
                    @endforeach
                ...
                <div class="form-group" id="vendor_field">
                    <label for="vendor_id">{{ trans('cruds.product.fields.vendor') }}</label>
                    <select class="form-control select2 {{ $errors->has('vendor') ? 'is-invalid' : '' }}" name="vendor_id"
                        id="vendor_id">
                        @foreach ($vendors as $id => $entry)
                            <option value="{{ $id }}"
                                {{ (old('vendor_id') ? old('vendor_id') : $product->vendor->id ?? '') == $id ? 'selected' : '' }}>
                                {{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <div class="needsclick dropzone {{ $errors->has('product_image') ? 'is-invalid' : '' }}"
                        id="product_image-dropzone">
                    </div>
...
@section('scripts')
    <script>
        ...
        $(document).ready(function() {
            // Initially hide the vendor field if product type is not 1
            if ($('input[name="product_type"]:checked').val() != 1) {
                $('#vendor_field').hide();
            }

            // Show/hide vendor field based on product type
            $('input[name="product_type"]').on('change', function() {
                if ($(this).val() == 1) {
                    $('#vendor_field').show();
                } else {
                    $('#vendor_field').hide();
                    //set the value of vendor_id to null
