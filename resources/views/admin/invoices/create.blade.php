    ...
    <div class="card-body">
        ...
        <hr>
            <div class="row">
                <div class="col-8">
                    <div class="form-group">
                        ...
                        <select class="form-control {{ $errors->has('products') ? 'is-invalid' : '' }} product_select" id="productSelect" >
                            @foreach($products as $id => $product)
                                <option data-unit-price="{{$product->product_price}}" value="{{ $product->id }}" {{ in_array($product->id , old('products', [])) ? 'selected' : '' }}>{{ $product->product_name }} - {{ $product->product_code }}</option>
                            @endforeach
                        </select>
                ...
                <div class="col-4">
                    <div class="form-group">
                        <label>.</label>
                        <button style="background: #321fdb;color: white;" class="form-control btn" type="button" id="addProductButton">Add Product</button>
                    </div>
                </div>
            </div>
            <div class="form-group">
                <table class="table table-bordered table-sm" id="productTable">
                    ...
                    <tr class="text-right">
                        ...
                        <td><h4 id="total_subtotal_price">0</h4></td>
                    </tr>
                    <tr class="text-right">
                        <td colspan="3" ><h4>Delivery</h4></td>
                        <td><h4 id="total_shipping_price">0</h4></td>
                    </tr>
            ...
            <div class="form-group d-none">
                ...
                <input class="form-control {{ $errors->has('total') ? 'is-invalid' : '' }}" type="number" name="total" id="order_total" value="{{ old('total', '0') }}" step="0.01" required>
                @if($errors->has('total'))
                    <div class="invalid-feedback">
                        {{ $errors->first('total') }}
                    </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $("#addProductButton").click(function () {
                ...
                const quantityInput = $("<input min='1' name='product[" + productCount + "][quantity]' type='number' class='form-control qty_input' value='1'>");
                ...
                var html = "<input name='product[" + productCount + "][product_price]' type='hidden' class='form-control product_price_' value='" + productDetails.product_price + "'>";
                html += "<input name='product[" + productCount + "][product_id]' type='hidden' class='form-control product_id' value='" + productDetails.id + "'>";
                html += "<input name='product[" + productCount + "][total_price]' type='hidden' class='form-control total_price_' value='" + total_price + "'>";
                $(tableRow).append(html);
                const removeButton = $("<span class='removeProduct badge badge-danger ' style='cursor:pointer;'>").text("Remove");
